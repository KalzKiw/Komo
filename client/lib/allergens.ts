...
export const ALLERGEN_VISUALS = [
  ...
  { keys: ["crustaceo", "crustaceos", "gamba", "langostino"], label: "Crustáceos", color: "#f97316", icon: "🦐" },
  ...
  { keys: ["sesamo", "sésamo"], label: "Sésamo", color: "#d97706", icon: "🌿" },
...
export type AllergenVisual = {
  label: string;
  color: string;
  icon: string;
};

export function allergenVisual(allergenName: string): AllergenVisual {
  ...
  return {
    label: allergenName ? String(allergenName).charAt(0).toUpperCase() + String(allergenName).slice(1) : "Alérgeno",
    color: "#64748b",
    icon: "•",
  };
}
