/** TypeScript port of frontend/sanitary.js — no vanilla JS deps */
...
const SANITARY_PRODUCT_INFO: SanitaryEntry[] = [
  {
  ...
  },
  {
    aliases: [
      ...
      "bocadillo tortilla de papas",
      "bocadillo de tortilla de papas",
      "tortilla de papas",
    ],
    ingredientes:
...
export function resolveSanitaryInfoReact(productName: string): SanitaryEntry {
  const normalized = normalizedText(productName);
  return (
    SANITARY_PRODUCT_INFO.find((entry) =>
      entry.aliases.some((alias) => normalized.includes(normalizedText(alias)))
    ) ?? FALLBACK
  );
}
