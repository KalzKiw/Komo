      ...
      <div className="space-y-4 px-4 py-5">
        ...
        <div className="overflow-hidden rounded-2xl bg-white shadow-sm">
          ...
          {token ? (
            <div className="px-4 pb-5 pt-3">
              <div className="flex items-center justify-center rounded-2xl bg-#d9f4ee py-6">
              ...
              <div className="mt-3 flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  ...
                  >
                    {copied ? <Check className="h-3.5 w-3.5 text-#1C9690" /> : <Copy className="h-3.5 w-3.5" />}
            ...
            <div className="px-4 pb-5 pt-3">
              <button
                ...
                onClick={generateToken}
                disabled={generatingToken}
                className="flex w-full items-center justify-center gap-2 rounded-2xl bg-#1C9690 py-3.5 font-bold text-white transition-all hover:bg-#169486 active:scale-[0.97] disabled:opacity-60"
              >
                {generatingToken ? (
        ...
        <div className="overflow-hidden rounded-2xl bg-white shadow-sm">
          <div className="flex items-center justify-between px-4 pb-2 pt-4">
            ...
            <span className="flex items-center gap-1 rounded-full bg-#c6efe7 px-2.5 py-1 text-xs font-semibold text-#169486">
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="mt-0.5 text-xs text-slate-400">
                        Saldo:{" "}
                        <span className="font-semibold text-#1C9690">{money(child.walletBalance)}</span>
                      </p>
                    </div>
                    <button
                      type="button"
                      onClick={() => setRechargeId(rechargeId === child.studentId ? null : child.studentId)}
                      className="rounded-xl border border-#92dbc8 bg-#d9f4ee px-3 py-1.5 text-xs font-semibold text-#169486 transition-all active:scale-95 hover:bg-#c6efe7"
                    >
                      Recargar
                    </button>
                  </div>
                  ...
                  {rechargeId === child.studentId && (
                    <div className="mt-3 flex items-center gap-2 rounded-xl bg-slate-50 p-3">
                      ...
                      <input
                        ...
                        className="w-24 rounded-xl border border-gray-200 bg-white px-3 py-1.5 text-sm font-semibold text-slate-800 focus:border-#44b6a1 focus:outline-none focus:ring-2 focus:ring-#2da38f/20"
                      />
                      <button
                        type="button"
                        disabled={recharging}
                        onClick={() => handleRecharge(child)}
                        className="flex-1 rounded-xl bg-#1C9690 py-1.5 text-sm font-bold text-white transition-all active:scale-95 hover:bg-#169486 disabled:opacity-60"
                      >
