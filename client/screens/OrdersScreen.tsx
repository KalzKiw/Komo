      ...
      <div className="h-full w-full bg-gray-50 overflow-y-auto [&::-webkit-scrollbar]:hidden" style={{ scrollbarWidth: "none" }}>
        ...
        <div className="sticky top-0 z-30 bg-white/80 backdrop-blur-md px-2 pt-2 pb-1 shadow-sm">
          <div className="flex items-center h-14">
            ...
            <span className={`rounded-full px-4 py-1 text-base font-semibold ml-auto ${detail.status === "PENDING" ? "bg-amber-100 text-amber-700" : detail.status === "CANCELLED" ? "bg-rose-100 text-rose-500" : "bg-emerald-100 text-emerald-700"}`}>{formatOrderStatus(detail.status)}</span>
          </div>
        </div>
        ...
        <div className="px-2 pt-2 pb-4">
          <div className="rounded-3xl bg-white shadow-lg px-6 py-6 flex flex-col items-center relative">
            {/* Número grande */}
            <span className="text-6xl font-extrabold text-emerald-700 tracking-widest mt-2 mb-2">{Number((detail as any).pickupNumber) || (100 + (parseInt(detail.id, 36) % 900))}</span>
            {/* Fecha y hora */}
            ...
            {/* Icono central */}
            <span className="flex items-center justify-center rounded-full bg-emerald-50 p-3 mb-2">
        ...
        <div className="px-2 pb-2">
          <div className="rounded-2xl bg-white shadow-sm mb-4 px-4 pt-4 pb-2">
              ...
              {detail.items.map((item, i) => (
                <div key={i} className="flex items-start gap-3 py-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <span className="text-base font-bold leading-snug text-emerald-700">{item.name}</span>
            ...
            <div className="border-t border-dashed border-gray-200 pt-3 mt-2">
              <div className="flex justify-between text-base mb-1">
                ...
                <span className="font-bold text-emerald-700">{money(detail.total)}</span>
              </div>
          ...
          <div className="text-center text-xs text-slate-400 mt-2 mb-4">
            Muestra este número (<span className="font-bold text-emerald-700">{Number((detail as any).pickupNumber) || (100 + (parseInt(detail.id, 36) % 900))}</span>) en el mostrador del comedor durante el turno seleccionado para recoger tu pedido.
          </div>
        </div>
  ...
  return (
    <div className="flex h-full flex-col bg-gray-50">
      ...
      <div className="flex-1 overflow-y-auto px-2 py-3 space-y-3">
            ...
            return (
              ...
              >
                ...
                <div className="flex flex-col items-end ml-2">
                  <span className={`text-lg font-bold ${order.status === "CANCELLED" ? "line-through text-slate-400" : "text-emerald-700"}`}>{money(order.total)}</span>
                </div>
              </button>
            );
          })}
        ...
        {filter === "IN_PROGRESS" && (
          <div className="mt-4 mb-2 rounded-2xl bg-emerald-700 p-5 text-white shadow-lg flex flex-col items-start">
            ...
            <button className="rounded-lg bg-white text-emerald-700 font-bold px-4 py-2 text-sm shadow hover:bg-emerald-50 transition">Ver Menú</button>
          </div>
        )}
      </div>
    </div>
      ...
      <div className="flex gap-2 overflow-x-auto pb-1">
        {suggestions.map((d) => (
          <button
            ...
            className={`flex flex-col items-center min-w-[90px] rounded-xl border border-gray-200 px-3 py-2 text-sm font-medium bg-white shadow-sm hover:bg-emerald-50 active:scale-95 transition-all ${loading === d.id ? 'opacity-60 pointer-events-none' : ''}`}
          >
            ...
            <span className="mt-1 text-xs text-emerald-700">Añadir</span>
          </button>
        ))}
      </div>
    </div>
